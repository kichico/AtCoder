...
void solve() {
    ll N, M; cin >> N >> M;
    createGraph takahashi(N), aoki(N);
    takahashi.inputAndAddEdge(M);
    aoki.inputAndAddEdge(M);
    vector<ll> ans(N);
    iota(ALL(ans), 0);
    do {
        bool flg = true;
        op(ans);
        rep(i, 0, N) {
            fore(j, takahashi.graph[i]) {
                // cout << ans[i] << " " << ans[j] << endl;
                if (aoki.graph[ans[i]].find(ans[j]) == aoki.graph[ans[i]].end()) {
                    cout << ans[i] << " " << ans[j] << endl;
                    flg = false;
                    break;
                }
