...
void dfs(ll now, graph& g, ll other) {
    if (visited[now]) return;
    visited[now] = true;
    fore(n, g.G[now]) {
        if (visited[n.to] || n.to == other) continue;
        cnt++;
        dfs(n.to, g, other);
    }
}
...
void solve() {
    ...
    auto pr1 = g.dijkstra(0);
    auto pr2 = g.dijkstra(N - 1);
    ll cf = 1, cs = 1;
    rep(i, 1, N - 1) {
        if (pr1.first[i] <= pr2.first[i]) cf++;
        else cs++;
    }
    twoText(cf > cs, "Fennec", "Snuke");
}
