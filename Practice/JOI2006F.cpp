...
int solve(){
    ...
    rep(i,info){
        ...
        string s;
        check.emplace_back();
		while (getline(ss, s, ' ')) check.back().emplace_back(stoi(s));
    }
    rep(i,info){
        check[i][1]--; check[i][2]--;
    }
    rep(i,info){
        //cout<<"here"<<endl;
        P p=make_pair(min(check[i][1],check[i][2]),max(check[i][1],check[i][2]));
        if(check[i][0]==1&&rec.count(p)==0) {
            ll cost=check[i][3];
            rec.emplace(p,cost);
        }
        else if(check[i][0]==1&&rec.count(p)==1) {
            ll cost=check[i][3];
            cost=min(cost,rec.at(p));
            rec.erase(p);
            rec.emplace(p,cost);
        }
        else if(check[i][0]==0) {
            ...
            for(auto& [pr,c]:rec){
                ...
                g.add_edge(pr.first,pr.second,c);
                g.add_edge(pr.second,pr.first,c);
            }
            //cout<<"debug2"<<endl;
            auto d=g.dijkstra(check[i][1]);
            if(d.first[check[i][2]]==INF) cout<<"-1"<<endl;
            else cout<<d.first[check[i][2]]<<endl;
        }
    }
    return 0;
}
