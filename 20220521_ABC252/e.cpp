...
void solve() {
    ...
    rep(i, 0, M) {
        ll from, to, cost; from--; to--;
        g.add_edge(from, to, cost);
        auto pr = make_pair(min(from, to), max(from, to));
        dict[pr] = i + 1;
    }
    auto pr = g.dijkstra(0);
    set<ll> ans;
    rep(i, 0, N) {
        auto rec = g.getpath(pr.second, i);
        rep(j, 0, rec.size() - 1) {
            auto pr = make_pair(min(rec[j], rec[j + 1]), max(rec[j], rec[j + 1]));
            if (dict.find(pr) == dict.end()) break;
            ans.emplace(dict[pr]);
            dict.erase(pr);
        }
    }
